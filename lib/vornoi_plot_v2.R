...
library(ggforce)
library(gganimate)
get_vornoi_plot=function(play_data,file_dir="output/vornoi_frames/"){
  # print(key)
  # play_data = df_play %>% filter(playId==key)
  gameid=play_data %>% pull(gameId) %>% head(1)
  index_temp=df_index %>% filter(gameId==gameid)
  home= df_games %>%  filter(gameId==gameid) %>% pull(homeTeamAbbr) %>% head(1)
  away=df_games %>% filter(gameId==gameid) %>% pull(visitorTeamAbbr) %>% head(1)
  desc=index_temp %>% pull(playDescription) %>% head(1)
  ...
  anim_save(paste0(file_dir,"play_anim_vornoy_gameid-",gameid,".mp4") , play_anim )
  play_anim
}
